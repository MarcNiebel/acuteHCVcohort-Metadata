...
#Making barplot of new variable
xlabels <- c("No","Yes")
pdf("Output/univariable_sc/IL28B_binary.pdf")
ggplot(combined_IL28B_levels,aes(x=as.factor(spont_clearance),fill=IL28B_genotype_binary))+
...
# two levels e.g. Yes/No or continous variable i.e. age
#Therefore infected genotype will be accessed separately
infected_genotype_separated <- combined_IL28B_levels[-c(17)]
...
infected_genotype_variables <- combined_IL28B_levels[c(1:3,17)]

#Infected genotype
infected_genotype_fit <-glm(spont_clearance ~ Infected_genotype, 
