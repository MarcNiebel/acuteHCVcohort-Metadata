...
#Purpose is to carry out survival analysis using KM plots
#and cox proportional hazard regression at univariable level
#including re-infected patients
...
all_patients_together_removed_gt2 <- all_patients_together %>%
    mutate(combined_genotype=case_when(combined_genotype =="gt2" ~ NA_character_,
                                       TRUE ~ combined_genotype))
write.csv(all_patients_together_removed_gt2,file="Output/survival_df.csv",row.names = FALSE)
