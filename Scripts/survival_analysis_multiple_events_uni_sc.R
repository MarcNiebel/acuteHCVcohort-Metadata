...
#taking into consideration re-infection
...
variable_data_cleaned <- variable_data_cleaned %>% mutate(Age=case_when(Age <41 ~ "<=40",
                                                                       TRUE ~ ">40"))
...
fits <- list(Age=formula_age,Gender=formula_gender,HIV=formula_hiv,
             PWID=formula_pwid,`HBV coinfection`=formula_hbv,`antiretroviral therapy`=formula_retroviral,
             ...
             `Infected Genotype`=formula_gt,IL28B=formula_il28b)
...
legend_titles <- list("Age","Gender","HIV status","PWID","Core HBV Ab",
                      ...
                      "IL28B")
legend_labs <- list(c("\u2264 40",">40"),c("Male","Female"),c("hiv positive","hiv negative"),c("No","Yes"),c("Negative","Positive"),
